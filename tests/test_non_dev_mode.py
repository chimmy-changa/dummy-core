...
def build_frontend_fixture():
    """A method to build react front-end and place it in matlab_web_desktop_proxy directory

    This method builds react front-end and copies the built files to matlab_web_desktop_proxy/gui.
    ...
    static_files_dir = os.path.join(os.getcwd(), "matlab_web_desktop_proxy", "gui")
...
def mock_settings_get_fixture(mocker):
    ...
    """
    mocker.patch(
        "matlab_web_desktop_proxy.settings.get",
        return_value=matlab_web_desktop_proxy.settings.get(dev=True),
    )
...
async def test_non_dev(test_server):
    ...
    copied into matlab_web_desktop_proxy/gui folder are added to the static_route_table of the server.
