...
The `matlab-web-desktop` Python® package allows you to integrate MATLAB® with Jupyter®. The MATLAB Web Desktop Proxy enables you to open a MATLAB desktop in a web browser tab, directly from your Jupyter environment. This is not a kernel integration.

The MATLAB Web Desktop Proxy is under active development and you might find issues with the MATLAB graphical user interface. For support or to report issues, see the [Feedback](#feedback) section.


## Use the MATLAB Web Desktop Proxy

Once you have a Jupyter environment with the `matlab-web-desktop` package installed, to use the integration, follow these steps:
...
5. To manage the MATLAB Web Desktop Proxy, click the tools icon shown below.
...
6. Clicking the tools icon opens a status panel with buttons like the ones below:
   ...
   * Feedback — Send feedback about the MATLAB Web Desktop Proxy. This action opens your default email application.
   * Help — Open a help pop-up for a detailed description of the options.


## Installation

The `matlab-web-desktop-proxy` package requires a Linux® operating system.
# TODO: Should we support this integration by default in a matlab image and remove the below content for this repo. But include it in the new jupyter-matlab-proxy ?
If you want to install this package in a Jupyter Docker® image, see [Use MATLAB Web Desktop Proxy in a Docker Container](https://github.com/mathworks-ref-arch/matlab-integration-for-jupyter/tree/main/matlab). Otherwise, if you want to install the `matlab-web-desktop` package into a preexisting Jupyter environment, follow the instructions below.

To install the `matlab-web-desktop-proxy` package, follow these steps in your Jupyter environment on a Linux OS:
# TODO: Should we support this by default in a matlab container and link it here ? 
...
4. Install the `matlab-web-desktop-proxy` package by executing:
```bash
python -m pip install matlab-web-desktop-proxy
```
...
If you want to use this integration with JupyterHub®, then you must install the `matlab-web-desktop` Python package in the Jupyter environment launched by your JupyterHub platform. For example, if your JupyterHub platform launches Docker containers, then install this package in the Docker image used to launch them. You can find a reference architecture that installs the `matlab-web-desktop` Python package in a Docker image in the repository [Use MATLAB Integration for Jupyter in a Docker Container](https://github.com/mathworks-ref-arch/matlab-integration-for-jupyter/tree/main/matlab).
